...
/**
  * Created by YS1607131010 on 8/10/2018.
  */
...
class EmptySet[A] extends MySet[A] {
  ...
  def --(setOne: MySet[A]): MySet[A] = this // difference
  def &(setOne: MySet[A]): MySet[A] = this // intersection
...
class NonEmptySet[A](head: A, tail: MySet[A]) extends MySet[A] {
  ...
  def --(anotherSet: MySet[A]): MySet[A] = filter(!anotherSet)
//  = {
//    def go(set: MySet[A], i: Int, j: Int) =
//      if (setOne(i) contains(setTwo(j))) {
//        set = set +(setTwo(j))
//      } else {
//        if (j == setTwo.size){
//          if (i == setOne.size){
...
object MySet {
  /*
     val s = MySet(1,2,3) = buildSet(seg(1,2,3), [])
     = buildSet(seq(2,3), [] + 1)
     = buildSet(seq(3), [1] + 2)
     = buildSet(seq(), [1, 2] + 3)
     = [1,2,3]
